 <?php
	...
	if($result->num_rows > 0)
	{
		...
		
		{
			...
			echo "<div id='rep".$a['userid']."'>";
		   $SQL2 = "select * from notification where friendid = ".$currentsuser;
			$result2=$conn->query($SQL2);
			if($result2->num_rows > 0)
			{
				while($c= $result2->fetch_assoc())
				{
					if(($post == $c['postid'])&&($c['userid']==$a['userid']))
					{      
						if($flag)
					...
					{ if($flag)
						 {echo "<button class='sbutton' style='background-color:#48d0ec; color:#ffffff' onclick='shareitem(".$a['userid'].",".$post.")'>share</button>";
						$flag = false;
					     }
					}
